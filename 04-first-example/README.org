...
- 本仓库优先通过根目录的 `run_moose_local.sh` 调用 MOOSE，避免每台机器的二进制路径都不一样
- 如果你的可执行文件不在 `PATH` 中，先设置 `MOOSE_LOCAL_BIN=/path/to/your-opt`
...
- 如果命令提示找不到 MOOSE，可先回到第二章检查 `combined-opt`、`moose-opt` 或 `MOOSE_LOCAL_BIN`
- 如果例子能运行但 ParaView 打不开结果，先确认 `cantilever_out.e` 是否真的生成
- 如果你在自己的 app 中复现本章，优先保证材料模块和固体力学模块已启用，再去排查输入文件
